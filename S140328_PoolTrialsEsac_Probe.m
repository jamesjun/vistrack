...
viAnimal_E = []; viSessionID_E = []; vrEODRq_E = [];  vrEODRz_E = [];
    ...
    for iAnimal = 1:4
        ...
        for iTrial = 1:4
            if (mlPath_E(iSession-viSession_E(1)+1, iTrial+(iAnimal-1)*4))
                ...
                vrEODRz_E = appendArray(vrEODRz_E, S.EODRz);
...
viAnimal_L = []; viSessionID_L = []; vrEODRq_L = []; vrEODRz_L = [];
    ...
    for iAnimal = 1:4
        ...
        for iTrial = 1:4
            if (mlPath_L(iSession-viSession_L(1)+1, iTrial+(iAnimal-1)*4))
                ...
                vrEODRz_L = appendArray(vrEODRz_L, S.EODRz);
...
getMatVal = @(mlMask, viY, viX) mlMask(sub2ind(size(mlMask), round(viY), round(viX)));
load mlMaskFood10;
% load mlMaskLandmark10;
vlVisit_E = getMatVal(mlMask, viY_E, viX_E);
vlVisit_L = getMatVal(mlMask, viY_L, viX_L);
...
vrImg_E1 = zeros(size(img0));
vrImg_L1 = zeros(size(img0));
vrImg_E1(sub2ind(size(img0), viY_E(vrEODRz_E>0), viX_E(vrEODRz_E>0))) = 1;
vrImg_L1(sub2ind(size(img0), viY_L(vrEODRz_L>0), viX_L(vrEODRz_L>0))) = 1;
vrImg_E1 = imfilter(vrImg_E1,H);
vrImg_L1 = imfilter(vrImg_L1,H);
...
figure; 
subplot 121; imshow(rgbmix(img0a, vrImg_E1, mlMask)); title('Early Visit counts');
subplot 122; imshow(rgbmix(img0a, vrImg_L1, mlMask)); title('Late Visit counts');
...
%%------------------------------------------------------------------
%% Fig. 4a. quantile shift location
...
% load mlMaskLandmark15;
load mlMaskFood5;
% load mlMaskFood5_15;
vlLoc_E = getMatVal(mlMask, vrY_E, vrX_E);
vlLoc_L = getMatVal(mlMask, vrY_L, vrX_L);
vpE = bootMean(vrEODRz_E(vlLoc_E));
vpL = bootMean(vrEODRz_L(vlLoc_L));
mr = [vpE, vpL];
...
%% EODR normalization (z-score)

figure; 
subplot 121; hold on; title('EOD rate (Hz)');
for i=1:numel(cvEODR)
    ksdensity(cvEODR{i}, 'function', 'survivor');
end
set(gca, 'XTick', [60:10:90])
axis([60 90 0 1])

subplot 122; hold on; title('EODR normalized (Z-score)');
for i=1:numel(cvEODRz)
    ksdensity(cvEODRz{i}, 'function', 'survivor');
end
set(gca, 'XTick', [-8:4:8])
axis([-8 8 0 1])

%% Fig. 4b. approach and depart EOD change

load mlMaskLandmark20_A;
xyLM1 = [966 418]; 
vlLoc_E = getMatVal(mlMask, viY_E, viX_E);
vlLoc_L = getMatVal(mlMask, viY_L, viX_L);
vrEODR_E1 = vrEODR_E(vlLoc_E);
vrEODR_L1 = vrEODR_L(vlLoc_L);
vrEODA_E1 = vrEODA_E(vlLoc_E);
vrEODA_L1 = vrEODA_L(vlLoc_L);
vrEODRq_E1 = vrEODRq_E(vlLoc_E);
vrEODRq_L1 = vrEODRq_L(vlLoc_L);
vrX_E1 = vrX_E(vlLoc_E);
vrY_E1 = vrY_E(vlLoc_E);
vrX_L1 = vrX_L(vlLoc_L);
vrY_L1 = vrY_E(vlLoc_L);
vrR_E1 = sqrt((vrX_E1 - xyLM1(1)).^2 + (vrY_E1 - xyLM1(2)).^2);
vrR_L1 = sqrt((vrX_L1 - xyLM1(1)).^2 + (vrY_L1 - xyLM1(2)).^2);
vrV_E1 = differentiate5(vrR_E1, .01);
vrV_L1 = differentiate5(vrR_L1, .01);
vrR_E1(vrV_E1<0) = -1 * vrR_E1(vrV_E1<0);
vrR_L1(vrV_L1<0) = -1 * vrR_L1(vrV_L1<0);

figure; imshow(img0a); hold on;
plot(vrX_E1(vrV_E1<0), vrY_E1(vrV_E1<0), 'r.');
plot(vrX_E1(vrV_E1>0), vrY_E1(vrV_E1>0), 'b.');
% viY_L, viX_L

pixpercm = 1053.28/(sqrt(2)*100);

% define negative vs positive distances
figure; plot(vrR_E1/pixpercm, vrV_E1/pixpercm, '.'); xlabel('Dist (cm)'); ylabel('Vel. (cm/s)');
figure; plot(vrR_E1(vrV_E1<0), vrEODR_E1(vrV_E1<0), '.'); xlabel('Dist (approach) early'); ylabel('EODR');
figure; plot(vrR_L1(vrV_L1<0), vrEODR_L1(vrV_L1<0), '.'); xlabel('Dist (approach) late'); ylabel('EODR');

%% seperate path and find the time of the minimum distance
load mlMaskLandmark20_A;
...
vrX = vrX_L;
vrEODR = vrEODA_L;
vlLoc = getMatVal(mlMask, round(vrY), round(vrX));


viStart = find(diff(vlLoc) > 0);
viEnd = find(diff(vlLoc) < 0);
viEnd = viEnd(viEnd > viStart(1));
n = min(numel(viStart), numel(viEnd));
viStart = viStart(1:n);
viEnd = viEnd(1:n);

%select approach trajectories
figure; imshow(img0a); hold on;
viTrajSlt = [];
mrEODR = [];
for iTraj = 1:numel(viStart)
    idxRng = viStart(iTraj):viEnd(iTraj);
    vr = sqrt((vrX(idxRng) - xyLM(1)).^2 + (vrY(idxRng) - xyLM(2)).^2) / pixpercm; %in cm    
    try
        iFirst = find(vr<5, 1, 'first');
        idxRng = viStart(iTraj) + iFirst - 1 + viTime;
        vr = vrEODR(idxRng);
        vr(~vlLoc(idxRng)) = nan;
        mrEODR(end+1, :) = vr;
        viTrajSlt(end+1) = iTraj;
        plot(vrX(idxRng), vrY(idxRng), '-', vrX(idxRng(50)), vrY(idxRng(50)), 'b.', vrX(idxRng(1)), vrY(idxRng(1)), 'r.');
    catch
        ;
    end   
end
figure; plot(viTime/100, nanmean(mrEODR)); xlabel('Time (s)'); ylabel('EODA (Hz/s)');
axis tight; grid on; 